...
from cubie.api.helpers.deps import resolve_dep_paths
from cubie.api.helpers.gpu_device import resolve_device_ids
...
async def resolve_model_definition_for_runtime(
...
async def build_model_runtime(
    ...
    model_definition = await resolve_model_definition_for_runtime(
        model_store,
    ...
    try:
        dep_paths = await resolve_dep_paths(
            model_id=model_id,
    ...
    resolved_device_ids = tuple(device_ids) if device_ids is not None else resolve_device_ids(config)
