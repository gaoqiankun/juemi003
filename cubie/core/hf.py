...
def ensure_hf_client_available() -> None:
...
def normalize_hf_endpoint(raw_value: Any, *, strict: bool) -> str:
...
def set_hf_endpoint(endpoint: str) -> str:
    normalized_endpoint = normalize_hf_endpoint(endpoint, strict=False)
...
def current_hf_endpoint() -> str:
    return normalize_hf_endpoint(os.environ.get(HF_ENDPOINT_ENV_KEY, HF_DEFAULT_ENDPOINT), strict=False)

def resolve_hf_status() -> tuple[bool, str | None]:
