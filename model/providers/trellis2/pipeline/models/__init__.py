...
def from_pretrained(path: str, **kwargs):
    ...
    Args:
        path: The local checkpoint prefix path.
              NOTE: config file and model file should be named f'{path}.json' and f'{path}.safetensors'.
    ...
    if not os.path.exists(config_file) or not os.path.exists(model_file):
        raise FileNotFoundError(
            f"TRELLIS2 model files not found: {config_file}, {model_file}. "
