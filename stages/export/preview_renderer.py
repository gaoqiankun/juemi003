...
def render_preview_png(model_path: Path, output_path: Path) -> None:
    ...
    center, radius = _scene_center_and_radius(trimesh_scene, np)
    scene = pyrender.Scene.from_trimesh_scene(
        trimesh_scene,
        bg_color=np.array([42 / 255, 42 / 255, 42 / 255, 1.0], dtype=float),
        ambient_light=np.array([0.18, 0.18, 0.18], dtype=float),
    )
    camera = pyrender.PerspectiveCamera(yfov=math.radians(45.0), aspectRatio=1.0)
    scene.add(camera, pose=_camera_pose(center, radius, np))

    for position, intensity in _light_rig(center, radius, np):
        scene.add(
            pyrender.PointLight(color=np.ones(3), intensity=intensity),
            pose=_translation_pose(position, np),
        )
    ...
    Image.fromarray(color, mode=image_mode).save(output_path, format="PNG")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print("usage: python -m gen3d.stages.export.preview_renderer <model.glb> <preview.png>", file=sys.stderr)
        return 2
