...
- 后续接口收口：`POST /v1/tasks` 的 `type` 现为可选，默认 `image_to_3d`；`model` 保持可选，默认 `trellis`
- 回归修复：`POST /v1/tasks` 不再触发任何模型加载；模型 runtime 构建改为 Worker 内后台线程执行，避免阻塞事件循环导致 504
- 验证结果：`python -m pytest tests -q` → `67 passed`
