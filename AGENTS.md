...
> 先读本文件，再读架构师在 Prompt 里指定的角色文件（`.claude/roles/`）。
...
- **开工前**在 `plan/` 新建 `YYYY-MM-DD-描述.md`（`Status: planning`），列明本次会改哪些文件；完成后改为 `Status: done`
- 若变更影响其他模块的行为，在 `.claude/decisions.md` 顶部追加一条
- 执行中遇到文档缺失、路径错误、流程不清晰等阻碍，在 `.claude/friction-log.md` 随手记一行
- 不升级依赖，除非明确要求
